...
import aula6_sistemaPedidos.*;

public class Main {
    public static void main(String[] args) {
        String caminhoParaDiretorioAtual = new File("").getAbsolutePath();

        String caminhoArquivo = caminhoParaDiretorioAtual + "\\aula6_sistemaPedidos\\produto.csv";
        List<Produto> produtos = ProdutoFileReader.lerArquivo(caminhoArquivo);

        caminhoArquivo = caminhoParaDiretorioAtual + "\\aula6_sistemaPedidos\\client.csv";
        List<Cliente> clientes = ClienteFileReader.lerArquivo(caminhoArquivo);
